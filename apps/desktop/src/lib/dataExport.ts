...
export function serializeSavedSkillAsSkillMd(skill: SavedSkill): string {
  ...
  lines.push(
    "---",
    `_Generated by ClearCapacity from observed work — estimated ~${formatDurationMinutes(skill.estimated_minutes_saved_per_week)} saved per week. Review before running._`,
    ""
  );
  return lines.join("\n");
}
