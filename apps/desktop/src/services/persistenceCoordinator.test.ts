...
test("React persistence exposes an awaitable barrier for the latest rendered snapshot", () => {
  ...
  assert.match(persistenceHookSource, /await writePersistedState\(latestSnapshotRef\.current\)/);
  assert.match(persistenceHookSource, /return\s+useMemo\(\(\)\s*=>\s*\(\{\s*flushLatest/);
...
test("reset awaits verified persistence deletion and reports it in the audit result", () => {
  ...
  assert.match(
    resetSource,
    /const persistedStateCleared\s*=\s*await clearPersistedState\(\)/,
  );
