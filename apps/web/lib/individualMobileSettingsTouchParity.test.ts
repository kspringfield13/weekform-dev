...
test("the narrow Individual drawer keeps Settings touch-ready beside its route targets", () => {
  ...
  assert.match(
    ...
    "the icon-only Download link must remain a full touch target in the mobile drawer",
  );
  assert.match(
    globalStyles,
    /@media \(max-width: 820px\)[\s\S]*?\.web-sidebar-footer-actions\s*\{[^}]*grid-template-columns:\s*44px minmax\(0, 1fr\)[^}]*\}/,
    "the mobile footer grid must reserve the Download link's full 44px target without overlapping Settings",
  );
});
