...
**Execution status (July 19, 2026):** Wave 0 planning is DONE — runbook Prompts 0A and 0B are executed. The five planning files exist in `docs/hackathon/` (`TEAM_CLAWFATHER_BASELINE.md`, `_PRODUCT_CONTRACT.md`, `_ARCHITECTURE.md`, `_TASKBOARD.md`, `_DECISIONS.md`), and the taskboard carries the full Program Integrator ledger. Per-task status now lives in `docs/hackathon/TEAM_CLAWFATHER_TASKBOARD.md`; see `docs/WEEKFORM_CODEX_PARALLEL_PROMPT_RUNBOOK.md` §0 for prompt-level status.
**Wave 2 status (July 19, 2026):** runbook Prompts 4–6 are implemented and gate-verified. Prompt 4 (copy-link invites) was already DONE. Prompt 5 (desktop Account & Sharing + manual sync — ledger DESK-01…05) landed under the sole Desktop Agent: `apps/desktop/src/services/cloudClient.ts`/`cloudPolicy.ts`/`cloudStore.ts`, `hooks/useCloudAccount.ts`/`useCloudSync.ts`, `components/settings/CloudAccountPanel.tsx`/`SharePreview.tsx`, plus the four high-conflict files; `npm run test:desktop-cloud` 37/37 (defensive parsers, backup excludes tokens, `clientSnapshotId` reuse, allowlist-only row mapping; a July 19 hardening pass added 17 `cloudClient.test.ts` and 8 `cloudStore.test.ts` tests over fixture responses and stubbed storage) and root `npm run build` exit 0. Prompt 6 (dashboards — ledger DASH-01…03): `apps/web/lib/workload.ts` + tests, `lib/snapshots.ts`, manager view at `app/teams/[teamId]/page.tsx` (medians + ranges, never sums; labeled prototype low-headroom threshold; stale/not-shared never rendered as zero; no leaderboards), member view at `app/dashboard/page.tsx`; `npm run test:web` 55/55 (one hardening pass added `snapshots.test.ts` — defensive row mapping, unshared metrics stay null not zero — and `teams.test.ts`; a second added mocked-Supabase-client tests for the query wrappers themselves: `listUserTeams`/`getOwnMembership`/`listTeamRoster`/`listTeamInvites` and `listLatestTeamSnapshots`/`listOwnLatestSnapshots`, covering embed normalization, role coercion, roster composition, and error paths) and `npm run web:build` exit 0 (10 routes). Honest limitation: no Supabase CLI/psql on this machine, so RLS/RPC behavior, live auth, and the four-account (Manager A / Member B / Member C / Outsider D) verification with screenshots remain unexercised against a live stack — those ledger rows sit at REVIEW, not DONE. **Wave 3 status (July 19, 2026):** runbook Prompts 7–9 are implemented and gate-verified via `npm run verify:wave3` (chains `test:desktop-cloud` 55/55, `test:web` 85/85, `web:build` 12 routes / 11 static pages; exit 0; independently re-run July 19, 2026). Prompt 7 (SYNC-01/02/03): pure `cloudScheduler.ts` (fingerprint no-op, startup catch-up, capped 1/5/15-min retries with `clientSnapshotId` reuse, 401/403 auth hard-stop, demo-mode exclusion; 18 tests) wired through `useCloudSync`/`CloudAccountPanel`. Prompt 8 (Team Briefing): server-only Responses-API path gated on `OPENAI_API_KEY`+`OPENAI_TEAM_BRIEFING_MODEL` (no guessed model ID, `store:false`), allowlist-only input, schema-validated output with invented evidence refs stripped, deterministic fallback from `workload.ts` risk flags, manager-only route with the AI disclosure; 20 fixture tests, zero live calls. Prompt 9: session-gated `/download` with honest public-source fallback plus a config-gated signed-URL route (`/download/artifact`, service key server-only). Honest limitations: live Supabase stack, live OpenAI model, live-app sync soak, and the private artifact bucket are all environment-blocked on this machine — see the runbook §0 rows for details. **Wave 1–3 validation pass (July 19, 2026):** an independent docs-vs-code audit and UI/ops review found no missing artifacts and no correctness bugs; it fixed stale provenance in `docs/BUILD_WEEK_2026.md` and the README's outdated "no automated test suite" claim, made `/download/artifact` failures redirect to a styled `/download?error=artifact` notice instead of raw JSON, and memoized the desktop cloud controller objects to stop per-render churn — all gates re-run green afterward (see the runbook §0 "Wave 1–3 validation & ops/UI polish pass" row). **Wave 3 execution hardening (July 19, 2026):** the Prompt 9 signed-URL branch — previously verified by inspection only — is now test-proven: the `/download/artifact` decision sequence was extracted into a pure dependency-injected planner (`planArtifactResponse` in `apps/web/lib/download.ts`, route reduced to a thin adapter) with 6 branch tests covering 503-unconfigured, 401-unauthenticated (service-key step provably never invoked), honest no-bucket 503, signing-failure 303 to the styled page, success 307 with exact-config pass-through, and a no-URL-leak-in-failures guard; `test:web` is now 91/91 and `verify:wave3` remains exit 0 (55/55 desktop-cloud, 12 routes / 11 static pages). A second hardening pass the same day did the equivalent for the Prompt 8 briefing orchestrator's untested branches — real-timer timeout/abort (signal proven wired), network-rejection fallback, `output[]`-array extraction, and an API-key-never-leaks guard — growing `test:web` to 95/95 with `verify:wave3` still exit 0. A third hardening pass the same day closed Prompt 7's last inspection-only claim — the desktop auto-sync real-timer wiring: the plan→timer translation moved out of the `useCloudSync` effect into the pure injectable `armAutoSyncTimer` (`TimerHost`) in `cloudScheduler.ts` with 5 new tests (exact-delay arming, `NOT_SCHEDULED` no-op disarm, stale-timer prevention on re-plan, the 1/5/15-minute ladder arming in order, and a real-platform-timer end-to-end fire), growing `test:desktop-cloud` to 60/60 with `verify:wave3` still exit 0 (102/102 web tests at that run).
**Wave 4 status (July 19, 2026):** runbook Prompts 11–12 are DONE at the repo level. Prompt 11 (integration and release gate): all runnable gates re-run as written on the final tree — `npm run test:cloud` 10/10, `npm run verify:wave3` exit 0 (60/60 desktop-cloud, 102/102 web, 12 routes / 11 static pages), root `npm run build` exit 0, secret scan clean — with the eight-dimension drift audit (verdict 8/8 CONSISTENT) recorded in `docs/hackathon/TEAM_CLAWFATHER_RELEASE_REPORT.md` §3.1. Prompt 12 (submission package): `docs/hackathon/TEAM_CLAWFATHER_DEMO_SCRIPT.md` (3:50 script, S1–S11 shot list, synthetic reset checklist), `docs/hackathon/TEAM_CLAWFATHER_DEVPOST.md` (tagline, full Devpost sections, submission checklist), the README team-cloud restructure, and the final `docs/BUILD_WEEK_2026.md` Wave 4 provenance entry. Ledger: SUB-01 DONE; SUB-02/SUB-03 REVIEW (recording and submit-side verification are human actions); SUB-04 PENDING (human-only submit); REL-01/REL-02 REVIEW (deploy and artifact signing operator-reserved). Honest limitations, never claimed as passed: live Supabase RLS four-actor verification, the ×2 synthetic golden path, and any live OpenAI call — operator-pending (no live credentials on this machine), per the runbook §0 Prompt 11 row and release report §5. The `npm audit` gate closed on July 19, 2026: `npm run audit:check` reports 0 vulnerabilities in both workspaces after remediating GHSA-qx2v-qp2m-jg93 with a `postcss >=8.5.10` override in `apps/web` (postcss 8.4.31 → 8.5.20), with all regression gates re-run green.
**Part 2 status (July 19, 2026):** Prompts 13–16 are complete at the repository level; Prompt 17 is READY. Prompt 16 uses manager-authorized security-definer RPCs for create, resolve/drop, and delete, with server-derived lifecycle fields and no authenticated direct table mutation. An additive SQL-review-only migration repairs databases that applied the earlier direct-write boundary, while the correlation-only team “what changed after” medians remain intact. The unapplied abuse contract contains 80 assertions covering direct table/column privileges, direct member UPDATE/DELETE, and outsider resolve/delete attempts. Final integration gates pass: focused 18/18, `verify:wave3` exit 0 with 133/133 desktop-cloud and 199/199 web tests, root build exit 0, `audit:check` exit 0 with zero vulnerabilities in both workspaces, and diff check exit 0. Live migration/RLS execution remains environment-blocked and is not claimed.
...
weekform-dev/
  ...
  supabase/
    ...
    seed.sql
  docs/
    TEAM_CLAWFATHER_ARCHITECTURE.md
    CLOUD_PRIVACY.md
```
...
> **Superseded for execution (July 19, 2026):** this table is the original planning breakdown. The Prompt 0B delivery ledger in `docs/hackathon/TEAM_CLAWFATHER_TASKBOARD.md` is authoritative for task IDs, estimates, and live status; it uses the execution-board ID scheme and carries a full §12→ledger ID mapping table at the top of its §1. Where its estimates differ from this table (e.g. Q-02 1.0h vs 1.5h here), the ledger wins.
...
A proposed project `absoloop.toml` should add real Weekform gates. Note (verified against `package.json`, July 19, 2026): only `npm run build` existed at planning time; `web:build` and `test:cloud` were added on July 19, 2026 by the Wave 1 Web scaffold and privacy-test tasks (ledger WEB-01 and CONTRACT-03) and both exit 0, so the `web_build` and `cloud_tests` gates below can now be enabled. `validate:cloud` (runs `test:cloud` then `web:build`) was added July 19, 2026 and exits 0, completing the §7.1 root-script set. Wave 2 (July 19, 2026) added two further passing gates usable in per-task missions: `npm run test:web` (55/55 — invite helpers + workload aggregates + snapshot row-mapping + team-role helpers + mocked-client tests of the Supabase query wrappers, all but the first two added in the July 19 Wave 2 hardening passes) and `npm run test:desktop-cloud` (37/37 — desktop cloud policy plus, from the same hardening pass, client parse/token-hygiene and store persistence tests). The Wave 2 close-out pass (July 19, 2026) added `npm run verify:wave2`, which chains `test:desktop-cloud`, `test:web`, and `web:build` as the single Wave 2 gate; it exits 0:
...
> **Part 2 prompt runbook (July 19, 2026):** the remaining Horizon 2/3 work
> (A7, Phase B, Phase C foundations) is now driven by
> [`docs/WEEKFORM_CODEX_PARALLEL_PROMPT_RUNBOOK_PT2.md`](WEEKFORM_CODEX_PARALLEL_PROMPT_RUNBOOK_PT2.md)
> — it opens with a full state-vs-blueprint gap analysis (its §1) and defines
> Prompts 13–21 across Waves 5–7, inheriting Part 1's evidence rules, honesty
> tags, and the standing `verify:wave3` / `build` / `audit:check` gates.
>
> **Detailed phased execution plan (July 19, 2026):** this section's three
> horizons are decomposed into an executable, dependency-ordered plan in
> [`docs/EXPANSION_ROADMAP.md`](EXPANSION_ROADMAP.md) (Phases A/B/C with
> acceptance criteria, env-blocked markers, and a status log). Execution has
> started: Phase A1 — manager planning scenarios ("What can the team
> absorb?") — is implemented as pure `apps/web/lib/scenario.ts` + tests plus
> a manager-view panel, and Phase A2 — weekly history and trend
> explanations — as `listTeamSnapshotHistory` (`apps/web/lib/snapshots.ts`),
> pure `apps/web/lib/trends.ts` + tests, and a self-baseline-labeled
> "Weekly trend" manager-view panel. Phase A3 — consent receipts and cloud
> retention controls — landed as pure
> `apps/desktop/src/services/consentReceipt.ts` (byte-exact, payload-derived
> receipts persisted, surfaced in the audit screen, and exportable) plus the
> web `apps/web/lib/retention.ts` single-constant retention statement; and
> Phase A6 — multiple per-team share policies — as the
> `teams.share_policy` migration (committed, not live-applied) with
> narrowing-only client-side enforcement (`applyTeamSharePolicy` desktop,
> `apps/web/lib/teamPolicy.ts` web mirror, adversarial widening tests).
> Combined gates July 19, 2026: `verify:wave3` exit 0 (86/86 desktop-cloud,
> 150/150 web, 12 routes), root build exit 0, `audit:check` 0
> vulnerabilities; see the roadmap's status log for full evidence. The
> Phase A exit criteria closed the same day: A4 landed as the
> `SessionStorageAdapter` seam (`apps/desktop/src/services/sessionStorage.ts`,
> capability-gated Keychain stub, 12 contract tests, live Keychain proof
> env-blocked) and A5 as the pairing specification + threat model
> (`docs/WEEKFORM_PAIRING_SPEC.md`, implementation deliberately deferred).
> Final gate: `verify:wave3` exit 0 — 97/97 desktop-cloud, 150/150 web.
> Phase A is complete: PT2 Prompt 13 shipped A7 forecasting. Prompt 14's
> experience-excellence pass, Prompt 15's optional local weekly-review ritual,
> and Prompt 16's manager action follow-through are DONE. Prompt 16 is hardened with
> SELECT-only table access, manager-authorized create/resolve/delete RPCs, and a
> 80-assertion abuse contract plus an additive migration for already-applied
> databases. Phase B Prompt 17 is READY. Focused 18/18,
> `verify:wave3` (133/133 desktop-cloud + 199/199 web), root build, audit
> (zero vulnerabilities in both workspaces), and diff checks pass. Live
> migration/pgTAP execution remains unclaimed.
>
> **Desktop/web alignment QA (July 19, 2026): CODE + STATIC GATES DONE;
> live RLS/API proof [env-blocked].** Parallel reviewers found and repaired
> stale-policy writes, remote-delete divergence, failed-retry deletion-guard loss, exhausted-retry state, native
> Store-to-localStorage split fallback, corrupt pending UUIDs, client-clock
> latest ordering, open-page staleness, and an unused browser Supabase client.
> Desktop uploads now fail closed behind a fresh membership/policy boundary and
> reconcile unchanged rows without automatically recreating a web deletion.
> Supabase owns `synced_at`; an additive migration upgrades already-provisioned
> databases, and the latest view and web freshness order on that
> receipt clock. Dashboard/team routes are force-dynamic and request fresh
> server/RLS data every 15 seconds while visible and online; auth cookies are
> the only persistent browser exception; there is no application-managed
> persistent browser workload cache. A credential-free revocation tombstone prevents a failed primary-store
> delete from resurrecting an old session; unconfirmed durable clear is surfaced.
> Gates: `verify:wave3` 127/127 desktop-cloud + 187/187 web, 12 routes;
> root build and diff check pass. Audit registry DNS, live migration/pgTAP, and
> desktop REST -> four-actor web visibility remain honestly environment-blocked.
...
1. ~~Create the baseline planning document from the baseline-mapper prompt.~~ **DONE (July 19, 2026):** shipped as `docs/hackathon/TEAM_CLAWFATHER_BASELINE.md` (the pack standardized on the `TEAM_CLAWFATHER_` prefix), alongside the other four Prompt 0A planning files and the Prompt 0B task ledger.
