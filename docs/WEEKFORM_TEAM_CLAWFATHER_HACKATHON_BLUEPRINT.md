...
**Part 2 status (July 19, 2026):** Prompts 13–16 are complete at the repository level; Prompt 17 is READY. Prompt 16 uses manager-authorized security-definer RPCs for create, resolve/drop, and delete, with server-derived lifecycle fields and no authenticated direct table mutation. The SQL-review-only migration and correlation-only team “what changed after” medians remain intact. The unapplied abuse contract contains 76 assertions covering direct member UPDATE/DELETE and outsider resolve/delete attempts. Final integration gates pass: focused 17/17, `verify:wave3` exit 0 with 128/128 desktop-cloud and 188/188 web tests plus 12 routes, root build exit 0, `audit:check` exit 0 with zero vulnerabilities in both workspaces, and diff check exit 0. Live migration/RLS execution remains environment-blocked and is not claimed.
...
> 76-assertion abuse contract. Phase B Prompt 17 is READY. Focused 17/17,
> `verify:wave3` (128/128 desktop-cloud + 188/188 web), root build, audit
