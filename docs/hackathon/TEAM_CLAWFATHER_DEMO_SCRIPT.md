...
This script demonstrates what actually runs, using a **local Supabase stack** (`supabase start`) that the operator brings up before recording. Live hosted Supabase, live RLS enforcement, the live OpenAI briefing model, and signed-artifact downloads were **not** exercised during development on this machine; the recording must not claim otherwise.
...
- **Download page:** without artifact env config, `/download` links to the **public source archive** and explains the build-from-source path (unsigned `.app`). Do not narrate a packaged/notarized download.
...
| 1:00–1:15 | Profile B — `/download` | Navigate to the authenticated download page; show the account-gated page and its build-from-source instructions (or signed-link button if artifact env is configured). | "Downloads sit behind the account. Today this prototype hands signed-in users the public source archive and a guided installer that builds an unsigned app locally — the gate is the distribution path; a packaged artifact drops in later without changing it." |
...
4. **S4 — Authenticated download page:** `/download` showing the account gate and the build-from-source (or signed-link) block.
