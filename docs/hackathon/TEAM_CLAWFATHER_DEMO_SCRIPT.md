...
- **Download page:** without a private artifact and complete signed/notarized/stapled/checksum proof, `/download` stays visibly pending and offers Weekform Web. The repository ships no public DMG fallback; do not narrate a downloadable or Gatekeeper-trusted Mac release until the external release gate succeeds.
- **Invites:** copy-link only. There is no email delivery; the UI says so.
- **Sync:** the individual chooses "Approve and start sharing," which durably records approval before starting the first sync and then enables hourly checks only while the app is open. Never say "always in sync" or "24/7".
...
| --- | --- | --- | --- |
| 0:00–0:20 | Profile A — weekform.dev `/login`, then `/dashboard` | Sign in as Avery. Dashboard shows the "Create team" form; type "Team Clawfather", submit; team card appears. | "Weekform is a local-first workload app for analysts — it already runs entirely on your Mac. This week we built the team layer on top of it. I'm Avery, a manager. I sign in on the web and create a team. Nothing about anyone's workload exists here yet — the dashboard is empty by design." |
...
| 1:00–1:15 | Profile B — `/download` | Navigate to the authenticated download page; show the pending release state and the useful Weekform Web action. | "The Mac release fails closed. This machine cannot produce a Developer ID signed and notarized build, so Weekform publishes no DMG and makes no Gatekeeper claim. The download appears only after signing, notarization, stapling, checksum verification, and private hosting are all attested." |
...
| 1:45–2:15 | Desktop — Settings → Account & Sharing | Show sharing off by default. Sign-in state, team selector, team rule, and individual selection. Expand **View exact data** and point at what is absent. | "Sharing is off by default and individually approved. The team rule can only narrow Blake's choices. Before anything uploads, Blake can inspect the exact JSON that will be sent — it's the same object the sync uploads, not a summary of it. Look at what's *not* in it: no window titles, no app names, no evidence, no notes, no calendar or chat details. A disabled metric is omitted — never sent as zero." |
| 2:15–2:30 | Desktop — Account & Sharing | Click **Approve and start sharing**; show the first-sync success status and local audit event. | "Blake approves once. Weekform stores that approval before the first versioned snapshot can upload under his own session, then checks hourly only while the app is open. There is no background daemon." |
...
| 3:15–3:40 | Desktop → Profile A | As Blake: open **Adjust sharing rules**, disable one metric (e.g., meetings), then approve the changed rules. Delete cloud history for the team. As Avery: refresh — the metric shows "Not shared", then the snapshot is gone. | "Approval is revocable. Blake turns off one metric — that requires a fresh individual approval — and Avery's view honors it after the approval-triggered sync. Then Blake deletes his cloud history entirely, and the manager view reflects that too. The member always holds the off switch." |
...
4. **S4 — Authenticated download page:** `/download` showing the account gate, fail-closed Mac release status, and Weekform Web continuation.
...
6. **S6 — Account & Sharing approval (hero shot):** team rule, individual selection, **Approve and start sharing**, and expanded **View exact data**. This is the single most important frame.
7. **S7 — First-sync success:** sync status + the local audit event entry.
...
**Final pre-flight:**
11. `npm run verify:wave3`, `npm run test:cloud`, and `npm run verify:web:release` pass at repo root; `npm --prefix apps/web run dev` serving.
