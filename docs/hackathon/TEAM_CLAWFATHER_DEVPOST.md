...
- **New this week — the Team Clawfather cloud slice:**
  - weekform.dev web app: signup/login, team creation, member management, single-use **copy-link invites** (no email provider by design), and an **account-gated Mac release page** that fails closed until a private DMG has explicit Developer ID, notarization, stapling, checksum, and verification-time proof. No public DMG fallback ships in the Web app.
  - Desktop **Account & Sharing**: off by default; sign-in, one recipient team, a narrowing-only team rule plus individual controls, an inspectable **exact JSON** candidate (the same object the sync sends), and one durable **Approve and start sharing** action that begins the first sync and bounded hourly checks only while the app is open. Cloud-history deletion and local audit events remain available for every cloud action.
...
**Known limitations disclosed to judges:** local pgTAP RLS proof does not establish linked/production state; live hosted auth/sync, the briefing model, and trusted artifact distribution remain unproven on the build machine (deterministic or fail-closed paths are demonstrated); copy-link invites only; prototype local state outside the Keychain/journal boundaries remains unencrypted; sync runs only while the app is open; capacity numbers are unvalidated heuristics; synthetic data only in all demo material.
