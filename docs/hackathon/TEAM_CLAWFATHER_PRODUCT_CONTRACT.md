...
4. **Approval on the Mac.** In Account & Sharing, sign in with the same account; sharing is **off by default**. Review or narrow the team-capped Summary + Categories rule, inspect the **exact payload**, and point out what is absent: window titles, raw activity, evidence, notes, screenshots. Choose **Approve and start sharing**; the durable individual approval starts the first sync. (§3.1 item 4, §5.5, §16.1.)
...
4. **Desktop account and sharing** — Account & Sharing settings; sign in with the same Supabase account; team selection; **sharing off by default**; exact payload preview before approval; share levels Summary / Categories / Projects; project names require an explicit allowlist; one durable individual approval starts the first sync and bounded in-app scheduling.
...
- **Cut order when the deadline bites** (§15 fallbacks): invite email and briefing AI go first (replaced by copy-link and deterministic briefing). Within P0: signed-artifact download degrades to an authenticated page linking the source ZIP with the limitation stated honestly (§7.7); desktop password auth degrades to a one-time device-pairing code (§4.2); a surveillance-feeling dashboard degrades to team-level aggregates only (§15). Individual approval before first upload, RLS, exact-data inspection, and revocation are never cut — without them there is no product thesis.
...
4. **Sharing is off by default** and gated on the member's explicit **Approve and start sharing** action (§3.1 item 4, §5.5).
